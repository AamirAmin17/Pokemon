...
export default function Home({ initialPokemon }: HomeData) {
  ...
  const filterPokemons = async (searchTerm: string) => {
    ...
    const filteredPokemons: IPokemonUrl[] = await response
      .json()
      .then((data) => {
  ...
  return (
    <Layout title="Pokemon">
      <div className="flex flex-col justify-center">
        <AiOutlineSearch className="absolute left-11 lg:ml-[11rem] text-black size text-xl" />
        <input
          type="text"
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-10 p-5">
        {pokemon.pokemonList.map((info, index) => (
          <div key={index}>
            <Pokemon
              name={info.name}
              index={info.id ? info.id + 1 : index + 1 + offset}
            />
          </div>
        ))}
      </div>
